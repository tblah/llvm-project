...
subroutine omp_do_lastprivate(a)
  ...
  ! CHECK-NEXT: omp.wsloop private(@{{.*}} %[[SHARED_A_1]] -> %[[A_PVT_REF:.*]], @{{.*}} %[[SHARED_I_1]] -> %[[I_PVT_REF:.*]] : !fir.ref<i32>, !fir.ref<i32>) {
  ...
  ! CHECK:        hlfir.assign %[[A_PVT_LOAD]] to %[[SHARED_A_1]] : i32, !fir.ref<i32>
  ! CHECK:      }
...
subroutine omp_do_lastprivate2(a, n)
  ...
  ! CHECK: omp.wsloop private(@{{.*}} %[[SHARED_A_2]] -> %[[A_PVT_REF:.*]], @{{.*}} %[[SHARED_N_2]] -> %[[N_PVT_REF:.*]], @{{.*}} %[[SHARED_I_2]] -> %[[I_PVT_REF:.*]] : !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>) {
  ...
  ! CHECK:   %[[A_PVT_LOAD:.*]] = fir.load %[[A_PVT_DECL]]#0 : !fir.ref<i32>
  ! CHECK:   hlfir.assign %[[A_PVT_LOAD]] to %[[SHARED_A_2]] : i32, !fir.ref<i32>
  ! CHECK:   %[[N_PVT_LOAD:.*]] = fir.load %[[N_PVT_DECL]]#0 : !fir.ref<i32>
  ! CHECK:   hlfir.assign %[[N_PVT_LOAD]] to %[[SHARED_N_2]] : i32, !fir.ref<i32>
  ! CHECK: }
...
subroutine omp_do_lastprivate_collapse2(a)
  ...
  ! CHECK:  omp.parallel shared(%[[ARG0_DECL]]#0 -> %[[SHARED_A_3:.*]], %[[I_IN_3:.*]]#0 -> %[[SHARED_I_3:.*]], %[[J_IN_3:.*]]#0 -> %[[SHARED_J_3:.*]] : !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>) {

  ! CHECK: %[[LB1:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: %[[UB1:.*]] = fir.load %[[SHARED_A_3]] : !fir.ref<i32>
  ! CHECK-NEXT: %[[STEP1:.*]] = arith.constant 1 : i32
  ! CHECK: %[[LB2:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: %[[UB2:.*]] = fir.load %[[SHARED_A_3]] : !fir.ref<i32>
  ! CHECK-NEXT: %[[STEP2:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: omp.wsloop private(@{{.*}} %[[SHARED_A_3]] -> %[[A_PVT_REF:.*]], @{{.*}} %[[SHARED_I_3]] -> %[[I_PVT_REF:.*]], @{{.*}} %[[SHARED_J_3]] -> %[[J_PVT_REF:.*]] : !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>) {
  ...
  ! CHECK:        %[[A_PVT_LOAD:.*]] = fir.load %[[A_PVT_DECL]]#0 : !fir.ref<i32>
  ! CHECK:        hlfir.assign %[[A_PVT_LOAD]] to %[[SHARED_A_3]] : i32, !fir.ref<i32>
  ! CHECK:      }
...
subroutine omp_do_lastprivate_collapse3(a)
  ...
  ! CHECK:  omp.parallel shared(%[[ARG0_DECL]]#0 -> %[[SHARED_A_4:.*]], %[[I_IN_4:.*]]#0 -> %[[SHARED_I_4:.*]], %[[J_IN_4:.*]]#0 -> %[[SHARED_J_4:.*]], %[[K_IN_4:.*]]#0 -> %[[SHARED_K_4:.*]] : !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>) {


  ! CHECK: %[[LB1:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: %[[UB1:.*]] = fir.load %[[SHARED_A_4]] : !fir.ref<i32>
  ! CHECK-NEXT: %[[STEP1:.*]] = arith.constant 1 : i32
  ! CHECK: %[[LB2:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: %[[UB2:.*]] = fir.load %[[SHARED_A_4]] : !fir.ref<i32>
  ! CHECK-NEXT: %[[STEP2:.*]] = arith.constant 1 : i32
  ! CHECK: %[[LB3:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: %[[UB3:.*]] = fir.load %[[SHARED_A_4]] : !fir.ref<i32>
  ! CHECK-NEXT: %[[STEP3:.*]] = arith.constant 1 : i32
  ! CHECK-NEXT: omp.wsloop private(@{{.*}} %[[SHARED_A_4]] -> %[[A_PVT_REF:.*]], @{{.*}} %[[SHARED_I_4]] -> %[[I_PVT_REF:.*]], @{{.*}} %[[SHARED_J_4]] -> %[[J_PVT_REF:.*]], @{{.*}} %[[SHARED_K_4]] -> %[[K_PVT_REF:.*]] : !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>, !fir.ref<i32>) {
  ...
  ! CHECK:        %[[A_PVT_LOAD:.*]] = fir.load %[[A_PVT_DECL]]#0 : !fir.ref<i32>
  ! CHECK:        hlfir.assign %[[A_PVT_LOAD]] to %[[SHARED_A_4]] : i32, !fir.ref<i32>
  ! CHECK:      }

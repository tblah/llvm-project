...
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_1]] -> %[[VAL_10:.*]] : !fir.ref<i32>) reduction(@neqv_reduction_l32 %[[SHARED_X_1]] -> %[[VAL_15:.*]] : !fir.ref<!fir.logical<4>>) {
...
! CHECK:             %[[VAL_13:.*]] = arith.constant 100 : i32
! CHECK:             %[[VAL_14:.*]] = arith.constant 1 : i32
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_2]] -> %[[VAL_10:.*]] : !fir.ref<i32>) reduction(@neqv_reduction_l32 %[[SHARED_X_2]] -> %[[VAL_15:.*]] : !fir.ref<!fir.logical<4>>) {
! CHECK-NEXT:          omp.loop_nest (%[[VAL_16:.*]]) : i32 = (%[[VAL_12]]) to (%[[VAL_13]]) inclusive step (%[[VAL_14]]) {
...
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_3]] -> %[[VAL_18:.*]] : !fir.ref<i32>) reduction(@neqv_reduction_l32 %[[SHARED_X_3]] -> %[[VAL_23:.*]], @neqv_reduction_l32 %[[SHARED_Y_3]] -> %[[VAL_24:.*]], @neqv_reduction_l32 %[[SHARED_Z_3]] -> %[[VAL_25:.*]] : !fir.ref<!fir.logical<4>>, !fir.ref<!fir.logical<4>>, !fir.ref<!fir.logical<4>>) {

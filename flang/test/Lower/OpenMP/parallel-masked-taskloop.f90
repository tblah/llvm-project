...
! CHECK:            omp.masked {
! CHECK:              omp.taskloop.context private(@[[I_PRIVATE]] %[[SHARED_I]] -> %[[ARG0:.*]] : !fir.ref<i32>) shared(%[[SHARED_J]] -> %[[TASK_SHARED_J:.*]] : !fir.ref<i32>) {
...
! CHECK:                    %[[LOAD_J:.*]] = fir.load %[[TASK_SHARED_J]] : !fir.ref<i32>
! CHECK:                    %c1_i32_1 = arith.constant 1 : i32
! CHECK:                    %[[RES_ADD:.*]] = arith.addi %[[LOAD_J]], %c1_i32_1 : i32
! CHECK:                    hlfir.assign %[[RES_ADD]] to %[[TASK_SHARED_J]] : i32, !fir.ref<i32>
! CHECK:                    omp.yield
! CHECK:                  }
! CHECK:                }

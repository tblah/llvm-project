...
! CHECK:           omp.parallel shared(%[[VAL_4]]#0 -> %[[SHARED_ARRAY:.*]], %[[VAL_6]]#0 -> %[[SHARED_I:.*]], %[[VAL_8]]#0 -> %[[SHARED_SCALAR:.*]] : !fir.ref<!fir.array<3x3xf64>>, !fir.ref<i32>, !fir.ref<f64>) {
! CHECK:             %[[VAL_11:.*]] = fir.embox %[[SHARED_ARRAY]](%[[PAR_SHAPE:.*]]) : (!fir.ref<!fir.array<3x3xf64>>, !fir.shape<2>) -> !fir.box<!fir.array<3x3xf64>>
...
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I]] -> %[[VAL_13:.*]] : !fir.ref<i32>) reduction(@add_reduction_f64 %[[SHARED_SCALAR]] -> %[[VAL_18:.*]], byref @add_reduction_byref_box_3x3xf64 %[[VAL_12]] -> %[[VAL_19:.*]] : !fir.ref<f64>, !fir.ref<!fir.box<!fir.array<3x3xf64>>>) {

...
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_1]] -> %[[VAL_7:.*]] : !fir.ref<i32>) reduction(@min_i32 %[[SHARED_X_1]] -> %[[VAL_12:.*]] : !fir.ref<i32>) {
...
! CHECK:             %[[VAL_10:.*]] = arith.constant 100 : i32
! CHECK:             %[[VAL_11:.*]] = arith.constant 1 : i32
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_2]] -> %[[VAL_7:.*]] : !fir.ref<i32>) reduction(@min_f32 %[[SHARED_X_2]] -> %[[VAL_12:.*]] : !fir.ref<f32>) {
! CHECK-NEXT:          omp.loop_nest (%[[VAL_13:.*]]) : i32 = (%[[VAL_9]]) to (%[[VAL_10]]) inclusive step (%[[VAL_11]]) {
...
! CHECK:             omp.wsloop private(@{{.*}} %[[SHARED_I_3]] -> %[[VAL_30:.*]] : !fir.ref<i32>) reduction(@min_f32 %[[SHARED_X_3]] -> %[[VAL_35:.*]] : !fir.ref<f32>) {

...
!CHECK:      omp.wsloop nowait schedule(runtime, simd) private(@{{.*}} %[[SHARED_IV]] -> %[[I_REF:.*]] : !fir.ref<i32>) {
!CHECK-NEXT:   omp.loop_nest (%[[I:.*]]) : i32 = (%[[WS_LB]]) to (%[[WS_UB]]) inclusive step (%[[WS_STEP]]) {
!CHECK:          %[[STORE:.*]]:2 = hlfir.declare %[[I_REF]] {uniq_name = "_QFEi"} : (!fir.ref<i32>) -> (!fir.ref<i32>, !fir.ref<i32>)

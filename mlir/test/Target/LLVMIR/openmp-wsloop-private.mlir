...
llvm.func @wsloop_private_(%arg0: !llvm.ptr {fir.bindc_name = "y"}) attributes {fir.internal_name = "_QPwsloop_private", frame_pointer = #llvm.framePointerKind<all>, target_cpu = "x86-64"} {
  ...
  omp.parallel shared(%5 -> %s5, %3 -> %s3, %1 -> %s1, %8 -> %s8, %7 -> %s7, %6 -> %s6 : !llvm.ptr, !llvm.ptr, !llvm.ptr, i32, i32, i32) {
    omp.wsloop private(@_QFwsloop_privateEc_firstprivate_ref_c8 %s5 -> %arg1, @_QFwsloop_privateEi_private_ref_i32 %s3 -> %arg2 : !llvm.ptr, !llvm.ptr) private_barrier reduction(@max_f32 %s1 -> %arg3 : !llvm.ptr) {
